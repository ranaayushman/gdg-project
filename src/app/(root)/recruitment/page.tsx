...
const TypewriterText = () => {
  ...
  return (
    <h2
      className={`mb-6 text-4xl font-bold text-gray-800 dark:text-white inline-block
      ${isComplete ? 'after:content-["_"] after:animate-pulse' : ""}
      ${bounce ? "animate-bounce" : ""}
      transition-all duration-300 transform
      ${
        isComplete
          ? "hover:scale-110 hover:text-blue-600 dark:hover:text-blue-400"
          : ""
      }
    `}
    >
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white/30 dark:bg-gray-900/30 backdrop-blur-md">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 py-16 text-center">
        ...
        {/* Main announcement */}
        <div
          className="max-w-2xl p-8 mx-auto mb-8 bg-white/70 dark:bg-gray-800/70 rounded-2xl shadow-lg backdrop-blur-sm
          transition-all duration-500 hover:shadow-2xl"
        >
          <TypewriterText />

          <p className="mb-6 text-lg text-gray-600 dark:text-gray-300 transition-opacity duration-500">
            Join us in building the future of technology at HIT. We&apos;re
            looking for passionate developers, designers, and tech enthusiasts.
          </p>

          {/* Info cards */}
          <div
            className={`grid grid-cols-1 gap-4 md:grid-cols-3 transition-all duration-500
            ${
              showCards
